...
from capsule.capsule import Capsule_Network, init_parameters
...
parser.add_argument('--num_pooling_layers', type=int, default=1)  # originally 2

parser.add_argument('--pooling_method', type=str, default='diffpool', choices=['diffpool', 'capsule'])

# capsule specific
parser.add_argument('--num_capsule_layers', type=int, default=1)
parser.add_argument('--capsule_mode', type=str, default='capsule', choices=['dummy', 'capsule'])
...
elif args.pooling_method == 'capsule':
    args.logdir = args.logdir + 'capsule'
    # just to make code runable
    linkpred = False
    if args.pooling_type == 'gnn':
        linkpred = True

    if args.capsule_mode == 'dummy':
        args.logdir = f'{args.logdir}/{args.dataset}/dummy/' \
                      f'{args.num_capsule_layers}_layers/{args.mlp_hidden_dim}_dim'
    elif args.capsule_mode == 'capsule':
        args.logdir = f'{args.logdir}/{args.dataset}/capsule/' \
                      f'{args.num_capsule_layers}_layers/{args.mlp_hidden_dim}_dim'
    if not os.path.exists(f'{args.logdir}'):
        os.makedirs(f'{args.logdir}')
    with open(f'{args.logdir}/summary.json', 'w') as f:
...
if args.pooling_method == 'diffpool':
    ...
    model = Capsule_Network(input_caps=stats['max_num_nodes'], output_caps=stats['num_classes'], n_iterations=2,
                            dim_upsample=96, dim_features=stats['num_features'], mlp_hidden_dim=args.mlp_hidden_dim,
                            max_num_nodes=stats['max_num_nodes'], num_layers=args.num_pooling_layers,
                            num_classes=stats['num_classes'], capsule_mode=args.capsule_mode).to(device)
...
if args.save:
    torch.save(best_model, f'{args.logdir}/models/{args.pooling_method}_{args.seed}.model')
...
}, f'{args.logdir}/{args.pooling_method}_{args.seed}.results')
