...
class Multi_Head_Graph_Pooling(nn.Module):
    ...
    def forward(self, u_predict, avg=False):
...
class CapsNet(nn.Module):
    def __init__(self, in_c=1, out_c=256, map_size=6, out_dim=784, routing_iterations=3, n_classes=10, reconstructed=False):
        super(CapsNet, self).__init__()
        
        self.conv1 = nn.Conv2d(in_c, out_c, kernel_size=3, stride=1)
        
        self.primaryCaps = PrimaryCapsLayer(out_c, int(out_c/8), 8, kernel_size=3, stride=2)
        
        self.num_primaryCaps = int(out_c/8) * map_size * map_size
                
        routing_module = Multi_Head_Graph_Pooling(int(out_c/8), map_size, n_classes, 16)
...
def load_caps_conv(args):
    model = CapsNet(in_c=3, out_c=256, map_size=14, out_dim=args.out_dim, reconstructed=args.reconstructed)
